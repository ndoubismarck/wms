...
const handleSubmit = async (data: VueFormData) => {
  ...
  let success = false
  let payload: LocationAisleModel | undefined
  try {
    const result = await app.services.locations.addAisle(data)
    await app.helpers.async.sleep(2000)
    if (result.success) {
      success = true
      payload = result.aisle
      form.value?.reset()
      alert.value = {
        type: EAlertMessageType.Success,
        body: {
          text: app.helpers.i18n.message('Aisle successfully added.'),
        },
      }
    } else {
      alert.value = {
        ...
        body: {
          text: app.helpers.i18n.message('Aisle could not be added.'),
        },
      }
    }
  } catch (ex) {
