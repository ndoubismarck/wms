...
const handleSubmit = async (data: VueFormData) => {
  ...
  let success = false
  let payload: LocationShelfModel | undefined
  try {
    data = { bay_id: bayId.value, ...data }
    const result = await app.services.locations.addShelf(data)
    await app.helpers.async.sleep(2000)
    if (result.success) {
      success = true
      payload = result.shelf
      form.value?.reset()
      alert.value = {
        type: EAlertMessageType.Success,
        body: {
          text: app.helpers.i18n.message('Shelf successfully added.'),
        },
      }
    } else {
      alert.value = {
        ...
        body: {
          text: app.helpers.i18n.message('Shelf could not be added.'),
        },
      }
    }
  } catch (ex) {
