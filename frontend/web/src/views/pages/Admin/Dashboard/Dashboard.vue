...
const getStats = async (): Promise<boolean> => {
  if (setup.value) {
    try {
      const lid = setup.value.location.id
      const result = await app.services.stats.get(lid)
      if (result.success) {
...
onMounted(async () => {
  greeting.value = new DateTime().greeting()
  ...
  <div class="dashboard-page">
      ...
      <div class="col-12 col-md-6 col-xl-8 mb-6 order-0">
        <div class="card h-100">
          <div class="d-flex align-items-start row">
            <div class="col-sm-9">
              <div class="card-body">
                <h5 class="card-title text-primary mb-3">{{ greeting }} {{ me?.firstName }}!</h5>
                <p class="mb-6">
                  You currently have <strong>{{ stats?.inventory.products.total }}</strong> products and
                  <strong>{{ stats?.inventory.productVariants.total }}</strong> product variants with
                  <strong>{{ stats?.inventory.stock.total }}</strong> items in stock.
                </p>
                <router-link :to="{ name: 'admin:inventory' }" class="btn btn-outline-primary">
                  <i class="bx bx-briefcase me-2"></i> Manage Inventory
                </router-link>
              </div>
            </div>
    ...
    <div class="row">
      <div class="col-12 col-md-12 order-0">
        <div v-if="stats" class="row h-100">
          <div class="col-12 col-md-6 col-xl-3 mb-6">
            <dashboard-stat-card
              title="Available Products"
              :value="stats.inventory.products.total"
              icon-class="bx bx-package text-primary"
              icon-bg-class="bg-label-primary"
              :week-trend="{
                change: stats.inventory.products.week_trend.change,
                direction: stats.inventory.products.week_trend.direction,
              }"
              :month-trend="{
                change: stats.inventory.products.month_trend.change,
                direction: stats.inventory.products.month_trend.direction,
              }"
              :annual-trend="{
                change: stats.inventory.products.annual_trend.change,
                direction: stats.inventory.products.annual_trend.direction,
              }" />
          </div>
          <div class="col-12 col-md-6 col-xl-3 mb-6">
            <dashboard-stat-card
              title="Available Product Variants"
              :value="stats.inventory.productVariants.total"
              icon-class="bx bx-cube text-info"
              icon-bg-class="bg-label-info"
              :week-trend="{
                change: stats.inventory.productVariants.week_trend.change,
                direction: stats.inventory.productVariants.week_trend.direction,
              }"
              :month-trend="{
                change: stats.inventory.productVariants.month_trend.change,
                direction: stats.inventory.productVariants.month_trend.direction,
              }"
              :annual-trend="{
                change: stats.inventory.productVariants.annual_trend.change,
                direction: stats.inventory.productVariants.annual_trend.direction,
              }" />
          </div>
          <div class="col-12 col-md-6 col-xl-3 mb-6">
            <dashboard-stat-card
              title="Available Stock"
              :value="stats.inventory.stock.total"
              icon-class="bx bx-box-alt text-success"
              icon-bg-class="bg-label-success"
              :week-trend="{
                change: stats.inventory.stock.week_trend.change,
                direction: stats.inventory.stock.week_trend.direction,
              }"
              :month-trend="{
                change: stats.inventory.stock.month_trend.change,
                direction: stats.inventory.stock.month_trend.direction,
              }"
              :annual-trend="{
                change: stats.inventory.stock.annual_trend.change,
                direction: stats.inventory.stock.annual_trend.direction,
              }" />
          </div>
          <div class="col-12 col-md-6 col-xl-3 mb-6">
            <dashboard-stat-card
...
<style scoped>
.dashboard-page .background-pattern {
  position: relative;
}

.dashboard-page .background-pattern::before {
  position: absolute;
