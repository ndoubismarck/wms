...
func Setup(ctx types.IContext, engine *gin.Engine, services *services.Services, helpers *helpers.Helper, middleware *middleware.Middleware, eventsChan chan *types.ServerEvent) error {
	...
	router.GET("/:lid/teams", mw.setup, mw.auth, h.getTeams)
	router.POST("/:lid/teams", mw.setup, mw.auth, mw.admin, h.addTeam)
	router.PUT("/:lid/teams/:id", mw.setup, mw.auth, mw.admin, h.updateTeam)
	router.GET("/:lid/teams/:id", mw.setup, mw.auth, h.getTeam)
	router.DELETE("/:lid/teams/:id", mw.setup, mw.auth, mw.admin, h.deleteTeam)
	router.GET("/:lid/teams/:id/users", mw.setup, mw.auth, h.getTeamMembers)
	router.PUT("/:lid/teams/:id/users", mw.setup, mw.auth, mw.admin, h.setTeamMembers)
	router.GET("/:lid/tasks", mw.setup, mw.auth, h.getTasks)
	router.POST("/:lid/tasks", mw.setup, mw.auth, mw.admin, h.addTask)
	router.PUT("/:lid/tasks/:id", mw.setup, mw.auth, mw.admin, h.updateTask)
	router.GET("/:lid/tasks/:id", mw.setup, mw.auth, h.getTask)
	router.DELETE("/:lid/tasks/:id", mw.setup, mw.auth, mw.admin, h.deleteTask)
