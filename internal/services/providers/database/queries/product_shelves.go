...
func (q *LocationShelves) FindCodes(params LocationShelvesParams) ([]string, error) {
	var rows []struct {
		Code string `gorm:"column:code"`
	}
	query := q.queryParams(params).Select("locations_shelves.code")
...
func (q *LocationShelves) FindMany(params LocationShelvesParams, pagination types.IPagination) ([]entities.LocationShelf, *types.PaginationResult, error) {
	...
	if err := query.
		Order(pagination.GetOrderWithPrefix("locations_shelves")).
